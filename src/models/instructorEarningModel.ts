...
interface InstructorEarningStatics {
  totalEarningsByInstructor(
    this: InstructorEarningModel,
    instructorId: string,
  ): Promise<number>;
  earningsByPeriod(
    this: InstructorEarningModel,
...
instructorEarningSchema.statics.earningsByPeriod = async function (
  instructorId: string,
  startDate: Date,
  endDate: Date,
) {
 ...
 * Middleware
 */
instructorEarningSchema.pre<Query<InstructorEarningDoc[], InstructorEarningDoc>>(
  /^find/,
  function (next) {
...
const InstructorEarning = model<InstructorEarningType, InstructorEarningModel>(
  'InstructorEarning',
