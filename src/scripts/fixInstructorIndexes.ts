...
async function dropIfExists(coll: mongoose.Collection, name: string) {
...
async function main() {
  ...
  await coll.createIndex(
    { userId: 1 },
    { unique: true, partialFilterExpression: { userId: { $type: 'objectId' } }, name: 'userId_1' },
  );
  console.log('  created partial unique userId_1');

  await coll.createIndex(
    { channelId: 1 },
    { unique: true, partialFilterExpression: { channelId: { $type: 'string' } }, name: 'channelId_1' },
  );
