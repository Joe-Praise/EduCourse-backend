...
export const getNotificationsForUser = catchAsync(
...
export const streamNotifications = (
  ...
  const listener = (notification: { userId?: { toString(): string } | string }) => {
    const targetId =
 ...
 */
export const getMyNotificationPreferences = catchAsync(
...
export const updateMyNotificationPreferences = catchAsync(
  async (req: AuthenticatedRequest, res: Response, _next: NextFunction) => {
    ...
    for (const type of PREF_TYPES) {
      const channel = body[type] as ChannelInput | undefined;
      if (channel && typeof channel === 'object') {
        if (typeof channel.inApp === 'boolean') update[`${type}.inApp`] = channel.inApp;
        if (typeof channel.email === 'boolean') update[`${type}.email`] = channel.email;
      }
    }
...
export function isPreferenceType(value: string): value is PrefType {
  return (PREF_TYPES as ReadonlyArray<string>).includes(value);
}
