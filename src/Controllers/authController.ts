...
export const refreshAccessToken = catchAsync(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const raw = (req as Request & { cookies?: Record<string, string> }).cookies?.[REFRESH_COOKIE_NAME];
    if (!raw) {
    ...
    if (doc.revoked) {
      ...
      logger.warn(
        `[auth] reuse of revoked refresh token for user ${doc.userId} — revoking all sessions`,
      );
