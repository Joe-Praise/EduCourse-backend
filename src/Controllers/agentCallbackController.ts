...
/** Upsert an Instructor record by channelId; returns its _id. */
interface ChannelEnrichment {
  avatarUrl?:       string;
  bio?:             string;
  customUrl?:       string;
...
async function upsertYouTubeInstructor(
  channelId: string,
  channelName: string,
...
// ---------------------------------------------------------------------------
// Per-course importer used by handleYouTubeImport
// ---------------------------------------------------------------------------
...
interface YouTubeCourseModule {
  title:       string;
  moduleIndex: number;
  videos:      YouTubeVideo[];
}

interface YouTubeImportedCourse {
  ...
  thumbnailUrl:      string;
  videoCount:        number;
  modules:           YouTubeCourseModule[];
  // Channel profile enrichment from the agent (optional)
  instructorAvatarUrl?: string;
  instructorBio?:       string;
  instructorCustomUrl?: string;
  subscriberCount?:     number;
}

async function importCourseFromYouTube(
  data: YouTubeImportedCourse,
  ...
  if (existingCourseId) {
    ...
    if (!updated) throw new Error(`Course ${existingCourseId} not found`);
...
export const handleYouTubeImport = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const body = req.body as {
      primaryCourseId?: string | number;
      courseQuery:      string;
      courses:          YouTubeImportedCourse[];
    ...
    logger.info(
      `[import] ← YouTube callback for course ${id}: ${body.courses.length} course(s) discovered for "${importQuery}"`,
    );
    ...
    for (let i = 0; i < body.courses.length; i++) {
      ...
      try {
        ...
        logger.info(
          `[import] ✓ "${courseData.title}" → ${courseId} (${courseData.modules.length} modules, ${lessonCount} lessons)`,
        );
        ...
        const lessonTitles = courseData.modules.flatMap((m) =>
          m.videos.map((v) => v.title),
        );
        ...
        logger.error(
          `[import] ✗ failed to import course index ${i} ("${courseData.title}"): ${reason}`,
        );
      }
    }
    ...
    if (importedIds.length === 0) {
      ...
      return next(
        new AppError(
          `All courses in the callback payload failed to import — ${failures.join(' | ')}`,
          500,
        ),
      );
    }
    ...
    logger.info(
      `[import] ✔ done — ${importedIds.length} course(s) published for "${importQuery}" (course cache invalidated)`,
    );
...
export const handleCourseSummary = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    ...
    const course = await Course.findByIdAndUpdate(
      id,
      { aiSummary: { difficulty, prerequisites, willBuild, summary } },
...
export const handleAutoTags = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { category: categoryName, tags } = req.body as {
      category?: string;
      tags?: string[];
    };
    ...
    if (Array.isArray(tags)) update.aiTags = tags;
    ...
    if (categoryName && !course.category) {
      ...
      if (name) {
        ...
        const cat = await Category.findOneAndUpdate(
          { name: new RegExp(`^${escaped}$`, 'i'), group: 'course' },
          { $setOnInsert: { name, group: 'course' } },
...
export const handleRecommendations = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { userId } = req.params;
    const { recommendations } = req.body as {
      recommendations: { courseId: string; reason?: string; order?: number }[];
    };
...
export const handleLearningPath = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { userId } = req.params;
    const { goal, path, estimatedWeeks, learningPathId } = req.body as {
      goal:           string;
      path:           { courseId: string; order: number; reason?: string }[];
...
export const handleQuiz = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { moduleIndex, questions } = req.body as {
      moduleIndex: number;
    ...
    await Quiz.findOneAndUpdate(
      { courseId: id, moduleIndex },
      { courseId: id, moduleIndex, questions, generatedAt: new Date(), status: 'ready' },
      { upsert: true, new: true },
    );
...
export const handleProgressNudge = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { userId } = req.params;
    const { message, courseId } = req.body as {
      message:  string;
      courseId?: string;
    };
...
export const handleReviewSentiment = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params;
    const { sentiment, flagged, moderationNote } = req.body as {
      sentiment?:      string;
      flagged?:        boolean;
    ...
    const review = await Review.findByIdAndUpdate(
      id,
      { sentiment, flagged, moderationNote },
      { new: true, runValidators: false },
    );

    if (!review) return next(new AppError('Review not found', 404));

    const reviewCourseId = (review as { courseId?: unknown }).courseId;
    if (!reviewCourseId) {
    ...
    if (sentiment === 'negative') {
      const windowStart = new Date(
        Date.now() - NEGATIVE_SENTIMENT_WINDOW_DAYS * 24 * 60 * 60 * 1000,
      );
      ...
      if (negativeCount >= NEGATIVE_SENTIMENT_THRESHOLD) {
        ...
        if (instructorId) {
          ...
          const linkedUser = (instructorDoc as { userId?: { _id?: unknown } } | null)?.userId;
