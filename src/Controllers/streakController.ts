...
async function buildActivityDateMap(userId: string): Promise<Map<string, number>> {
  ...
  for (const d of [...completionDates, ...enrollmentDates]) {
    dateMap.set(d._id as string, (dateMap.get(d._id as string) ?? 0) + (d.count as number));
  }
  return dateMap;
}

function computeStreaks(dateMap: Map<string, number>): {
...
export const getLearningStreak = catchAsync(
  async (req: AuthenticatedRequest, res: Response, _next: NextFunction) => {
    const userId = (req.query.userId as string | undefined) ?? req.user!._id;
...
export const getUserBadges = catchAsync(
  async (req: AuthenticatedRequest, res: Response, _next: NextFunction) => {
    ...
    const enrollmentCount: number = enrollments[0]?.total ?? 0;
    const reviewCount: number = reviews[0]?.total ?? 0;
    const completionCount: number = completions[0]?.total ?? 0;
