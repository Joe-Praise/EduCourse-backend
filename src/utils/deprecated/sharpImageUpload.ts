...
// Self-contained minimal authed-request shape so this file can be lifted
// into another project without depending on the rest of the codebase.
 ...
 */
export const userResizePhoto = catchAsync(
 ...
 */
export const courseResizePhoto = catchAsync(
  async (req: AuthedRequest, _res: Response, next: NextFunction) => {
    if (!req.file) return next();

    (req.file as any).filename = `course-${req.user!._id}-${Date.now()}.jpeg`;
 ...
 */
export const blogResizePhoto = catchAsync(
