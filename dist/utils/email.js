...
const getClient = () => {
    ...
    if (!apiKey) {
        throw new AppError("Email service is not configured. RESEND_API_KEY is missing.", 500);
    }
    return new Resend(apiKey);
};
const getFromAddress = () => process.env.RESEND_FROM_EMAIL || "onboarding@resend.dev";
const resolveRecipient = (intendedTo) => process.env.RESEND_TO_EMAIL || intendedTo;
...
`;
const buildResetEmailText = (name, resetUrl) => `Hi ${name || "there"},\n\n` +
    `We received a request to reset the password on your EduCourse account.\n` +
