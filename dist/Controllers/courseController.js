...
export const atlasAutocomplete = catchAsync(async (req, res, next) => {
    const { q } = req.query;
    const pipeline = [];
    ...
    if (slug) {
        ...
        if (doc.length === 0 &&
            ...
            if (existing) {
                return res.status(200).json({
                    status: 'success',
                    data: [existing._doc ?? existing],
                    importing: existing.publishedStatus === 'importing',
                });
            }
            logger.info(`[import] no catalog match for "${searchQuery}" → creating draft + triggering YouTube discovery`);
            ...
            await safeTriggerAgent('youtube-course-discovery', {
...
export const getLectureCourse = catchAsync(async (req, res, next) => {
    ...
    const completedLessons = (effectiveCompleted?.lessonsCompleted ?? []).map((id) => id.toString());
...
export const getMyLearningCourse = catchAsync(async (req, res, next) => {
    const userId = req.user._id;
...
export const searchMyLearningCourse = catchAsync(async (req, res, next) => {
    const { q } = req.query;
    const userId = req.user._id;
    if (!userId) {
...
export const resizePhoto = catchAsync(async (req, _res, next) => {
    ...
    if (!courseId) {
        return next(new AppError('Course ID is required to upload a cover image.', 400));
    }
...
export const uploadResources = catchAsync(async (req, res, next) => {
    if (!req.file) {
...
export const submitForReview = catchAsync(async (req, res, next) => {
    const course = await Course.findByIdAndUpdate(req.params.id, { publishedStatus: 'review', submittedForReviewAt: new Date() }, { new: true, runValidators: true }).setOptions({ skipPublishedFilter: true });
    if (!course) {
...
export const publishCourse = catchAsync(async (req, res, next) => {
    const course = await Course.findByIdAndUpdate(req.params.id, { publishedStatus: 'published', publishedAt: new Date() }, { new: true, runValidators: true }).setOptions({ skipPublishedFilter: true });
    if (!course) {
        return next(new AppError('No course found with that ID', 404));
    }
    appEvents.emit(CacheEvent.COURSE.UPDATED, course);
...
export const archiveCourse = catchAsync(async (req, res, next) => {
    const course = await Course.findByIdAndUpdate(req.params.id, { publishedStatus: 'archived' }, { new: true, runValidators: true }).setOptions({ skipPublishedFilter: true });
    if (!course) {
        return next(new AppError('No course found with that ID', 404));
    }
    appEvents.emit(CacheEvent.COURSE.UPDATED, course);
