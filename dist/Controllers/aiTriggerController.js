...
// Recommendations
// ---------------------------------------------------------------------------
export const triggerRecommendations = catchAsync(async (req, _res, next) => {
    ...
    await Recommendation.findOneAndUpdate({ userId }, { $set: { status: 'pending', userId } }, { upsert: true, new: true, setDefaultsOnInsert: true });
    ...
    const completedCourseIds = completed
        ...
        .filter(Boolean);
    const triggered = await safeTriggerAgent('course-recommender', {
        userId,
...
export const getRecommendations = catchAsync(async (req, res, _next) => {
    const userId = req.user._id.toString();
    const cached = await cacheManager.get(`ai:recommendations:${userId}`);
    if (cached) {
...
// Learning Path
// ---------------------------------------------------------------------------
export const triggerLearningPath = catchAsync(async (req, res, next) => {
    const userId = req.user._id.toString();
    const { goal } = req.body;
    ...
    const triggered = await safeTriggerAgent('learning-path-builder', {
        userId,
        goal: goal.trim(),
...
export const getLearningPath = catchAsync(async (req, res, next) => {
    const { id } = req.params;
...
export const getMyLearningPaths = catchAsync(async (req, res, _next) => {
    const userId = req.user._id.toString();
...
// Quiz Generation
// ---------------------------------------------------------------------------
export const triggerQuizGeneration = catchAsync(async (req, res, next) => {
    ...
    const moduleDoc = courseModule;
    const lessonTitles = (moduleDoc.lessons ?? []).map((l) => l.title);
    if (lessonTitles.length === 0) {
        return next(new AppError('Module has no lessons to generate a quiz from', 400));
    }
    // Upsert pending marker
    await Quiz.findOneAndUpdate({ courseId: id, moduleIndex: modIdx }, { $set: { status: 'pending', courseId: id, moduleIndex: modIdx } }, { upsert: true, new: true, setDefaultsOnInsert: true });
    const triggered = await safeTriggerAgent('quiz-generator', {
        courseId: id,
        moduleIndex: modIdx,
    ...
    if (!triggered) {
        await Quiz.updateOne({ courseId: id, moduleIndex: modIdx }, { $set: { status: 'failed' } });
        return next(new AppError('Agent service unavailable. Try again shortly.', 503));
    }
    res.status(202).json({
        status: 'success',
...
export const getQuiz = catchAsync(async (req, res, _next) => {
    const { id, moduleIndex } = req.params;
    const modIdx = parseInt(moduleIndex, 10);
...
//# sourceMappingURL=aiTriggerController.js.map