 ...
 */
export const getAllInstructors = catchAsync(async (req, res, _next) => {
    // Distinct instructor ids referenced by any published course.
    const instructorIds = await Course.distinct('instructors', {
        publishedStatus: 'published',
    });
    const features = new APIFeatures(Instructor.find({ _id: { $in: instructorIds } }), req.query)
        .filter()
        .sorting()
        .limitFields();
...
export const getMyLearningInstructors = catchAsync(async (req, res, next) => {
...
export const suspendInstructor = catchAsync(async (req, res, next) => {
    const { id } = req.params;
