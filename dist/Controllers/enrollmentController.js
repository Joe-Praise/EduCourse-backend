...
export const createEnrollment = catchAsync(async (req, res, next) => {
    ...
    if (instructor) {
        ...
        const amount = typeof courseData === 'object' && courseData?.price ? courseData.price : 0;
        if (amount > 0) {
...
export const deleteEnrollment = catchAsync(async (req, res, next) => {
    const doc = await Enrollment.findByIdAndUpdate(req.params.id, { active: false }, { new: true });
    if (!doc) {
