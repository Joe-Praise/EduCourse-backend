...
export const getEarningsByInstructor = catchAsync(async (req, res, next) => {
...
export const getDashboardOverview = catchAsync(async (req, res, next) => {
    ...
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const [totalStudents, totalEarnings, thisMonthEarnings, recentEnrollments, byCourse] = await Promise.all([
    ...
    const courses = await Instructor.findAllByInstructor
        ? []
        : [];
    const overview = {
...
export const getEngagementHeatmap = catchAsync(async (req, res, next) => {
    const instructor = await Instructor.findOne({ userId: req.user._id });
    if (!instructor) {
        return next(new AppError('Instructor profile not found', 404));
    }
    const instructorId = instructor._id;
    const cacheKey = CacheKeyBuilder.resourceKey('instructor-engagement', instructorId.toString());
    const cached = await cacheManager.get(cacheKey);
    if (cached) {
        return res.status(200).json({ status: 'success', data: cached });
    }
...
//# sourceMappingURL=instructorEarningController.js.map