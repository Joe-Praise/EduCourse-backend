...
async function main() {
    ...
    const instructors = await coll
        ...
        .toArray();
    console.log(`Found ${instructors.length} YouTube instructor(s) to ${REFRESH_ALL ? 'refresh' : 'backfill'}.`);
    if (instructors.length === 0) {
        await mongoose.disconnect();
        return;
    }
    const channelIds = [
        ...new Set(instructors.map((i) => i.channelId).filter(Boolean)),
    ];
    ...
    for (const ins of instructors) {
        ...
        if (profile.avatarUrl)
            set.channelThumbnailUrl = profile.avatarUrl;
        if (profile.channelUrl)
            set.channelUrl = profile.channelUrl;
        if (profile.subscriberCount)
            set.subscriberCount = profile.subscriberCount;
...
//# sourceMappingURL=backfillYouTubeInstructors.js.map