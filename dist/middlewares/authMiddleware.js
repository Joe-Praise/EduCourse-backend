...
import { PERMISSION_MATRIX } from '../utils/constants.js';
 ...
 */
export const restrictTo = (roles, options = {}) => {
...
export const requirePermission = (resource, action) => {
    return async (req, res, next) => {
        ...
        if (!isPermitted) {
            ...
            return next(new AppError(`Access denied: You cannot '${action}' on '${resource}'. Required roles: [${PERMISSION_MATRIX[resource]?.[action]?.join(', ') || 'undefined'}]`, 403));
        }
        // Success logging
 ...
 */
export const requireOwnerOrRole = (roles) => {
    return async (req, res, next) => {
        if (!req.session?.user) {
            return next(new AppError('Authentication required', 401));
        }
 ...
 */
export const getPermissionCacheStats = async () => {
    try {
        const pattern = `${CACHE_PREFIX}*`;
        const keys = await redis.keys(pattern);
        const keysByUser = {};
